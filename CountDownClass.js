...
let timerID = null
let path = "https://osmanmez.github.io/Socces-ScoreBoard/sounds/audio.mp3";
        ...
        render() {
            ...
            return ( <
                ...
                p className = { `${!extra ? "stopwatch-time" : "stopwatch-time2"}` } > { `${time.hours.toString().padStart(2, '0')}:${time.minutes.toString().padStart(2, '0')}:${time.seconds.toString().padStart(2, '0')}` } < /p>  {
                    ...
                    onClick = {
                            () => this.setState({ paused: !paused }) } > { paused ? 'CONTINUAR' : 'DETENER' } < /button>  <
                        button disabled = { disabled }
                    onClick = {
                            () => this.reset() } > INICIAR < /button> <
                        /p>  <
                        /div>
                );
            }
